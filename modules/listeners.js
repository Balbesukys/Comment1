...
export const userLike = () => {
  ...
  const addLikesListener = document.querySelectorAll(".like-button");
  for (const likeButtonElement of addLikesListener) {
...
export const addCommentListener = () => {
  ...
  for (const commentElement of cemmentElements) {
    commentElement.addEventListener("click", () => {
      const addCommentListener = commentElement.dataset.quote;
      textInput.value = `"${userComments[addCommentListener].author.name}"\n"${userComments[addCommentListener].text}"`;
    });
  }
};
...
export const addReplyCommentListener = () => {
